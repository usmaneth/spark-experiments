#!/usr/bin/env bash
# Detached: for each new bonsai2_dspark_full2_step*.safetensors, convert (two-step + Q4_K_M) and eval on node_b (K=5 exact + tau 0.05).
V2=/home/REDACTED/Bonsai-demo/dflash-training/v2; MD=/home/REDACTED/Bonsai-demo/models/bonsai2-dspark; ROOT=/home/REDACTED/Bonsai-demo; DONOR=$ROOT/models/bonsai2-gguf/27B/Ternary-Bonsai-2-27B-PQ2_0.gguf; sz(){ stat -c%s "$1" 2>/dev/null || echo 0; }
...
for i in $(seq 1 10080); do
  for CK in $(ls $MD/bonsai2_dspark_full2_step*.safetensors 2>/dev/null); do
    ...
    rsync -a "$Q" node_b:$MD/ 2>/dev/null; for j in $(seq 1 30); do n=$(ssh -o ConnectTimeout=8 node_b 'nvidia-smi --query-compute-apps=pid,process_name --format=csv,noheader 2>/dev/null | grep -v llama-server | wc -l'); [ "${n:-1}" -eq 0 ] && break; sleep 60; done
    n=$(ssh -o ConnectTimeout=8 node_b 'nvidia-smi --query-compute-apps=pid,process_name --format=csv,noheader 2>/dev/null | grep -v llama-server | wc -l'); echo "[$(date +%H:%M:%S)] CKPT $N: eval on node_b ($([ "${n:-0}" -gt 0 ] && echo CONTENDED || echo clean))"
    ...
    scp -q /tmp/${TAG}_sweep.sh node_b:/tmp/${TAG}_sweep.sh && ssh node_b "bash /tmp/${TAG}_sweep.sh" | grep -E '^RESULT|DONE'
