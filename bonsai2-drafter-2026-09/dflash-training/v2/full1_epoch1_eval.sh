#!/usr/bin/env bash
# Detached: when the epoch-1 Q4_K_M exists AND spark2's GPU is empty, run the clean sweep there.
V2=/home/usman/Bonsai-demo/dflash-training/v2; MD=/home/usman/Bonsai-demo/models/bonsai2-dspark
Q=$MD/bonsai2-dspark-full1ep1-Q4_K_M.gguf; TAG=full1ep1
echo "[$(date +%H:%M:%S)] eval waiter started (needs $Q + idle spark2; cap 6h)"
...
echo "[$(date +%H:%M:%S)] EPOCH1 GGUF present ($(stat -c%s "$Q") bytes); copying to spark2"; rsync -a "$Q" spark2:$MD/ 2>&1 | tail -1
for i in $(seq 1 5); do n=$(ssh -o ConnectTimeout=8 spark2 'nvidia-smi --query-compute-apps=pid --format=csv,noheader 2>/dev/null | wc -l'); [ "${n:-1}" -eq 0 ] && break; echo "[$(date +%H:%M:%S)] spark2 busy ($n procs), courtesy wait"; sleep 60; done; n=$(ssh -o ConnectTimeout=8 spark2 'nvidia-smi --query-compute-apps=pid --format=csv,noheader 2>/dev/null | wc -l'); TAG_C="clean"; [ "${n:-0}" -gt 0 ] && TAG_C="CONTENDED(${n} other GPU procs; tok/s invalid, acceptance valid)"; echo "[$(date +%H:%M:%S)] GPU state: $TAG_C"
echo "[$(date +%H:%M:%S)] LAUNCH clean sweep on spark2 (K=4,5; p_min=0; PQ2_0)"
cat > /tmp/ep1_sweep.sh <<'EOS'
cd /home/usman/Bonsai-demo; export LD_LIBRARY_PATH=/home/usman/Bonsai-demo/bin/cuda
...
EOS
scp -q /tmp/ep1_sweep.sh spark2:/tmp/ep1_sweep.sh && ssh spark2 'bash /tmp/ep1_sweep.sh'
