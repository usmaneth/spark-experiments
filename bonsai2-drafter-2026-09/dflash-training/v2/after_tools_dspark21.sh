#!/usr/bin/env bash
# node_b: after the tool/agent set is generated, extract it in the five-tap BON2 format and
# fine-tune the DSpark step-600 weights on it (DSpark v2.1). Runs unattended.
set -u
ROOT=/home/REDACTED/Bonsai-demo; V2=$ROOT/dflash-training/v2; MD=$ROOT/models/bonsai2-dspark
...
cd $ROOT && sudo docker run --rm --gpus all --ipc=host --name dspark_v21 -v /home/REDACTED:/home/REDACTED bonsai/dflash-trainer:latest \
