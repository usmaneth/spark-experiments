#!/usr/bin/env bash
# EAGLE-3 smoke checkpoint: wait for the spark2 training container to exit, convert on spark1,
# quantize Q8_0 and Q4_K_M, push to spark2, run the K=5 sweep on a clean spark2 GPU.
...
gpu_busy(){ ssh -o ConnectTimeout=8 spark2 'nvidia-smi --query-compute-apps=pid,process_name --format=csv,noheader 2>/dev/null | grep -v llama-server | wc -l'; }

say "EAGLE3 $TAG eval: waiting for container $CONT to exit"
for i in $(seq 1 240); do
  ssh -o ConnectTimeout=8 spark2 "sudo docker ps -q --no-trunc | grep -q '^$CONT'" || break; sleep 30
done
for i in $(seq 1 20); do
  s=$(ssh -o ConnectTimeout=8 spark2 "stat -c%s $CK 2>/dev/null || echo 0"); [ "$s" -gt 3000000000 ] && break; sleep 15
done
[ "${s:-0}" -gt 3000000000 ] || fail "checkpoint missing or small on spark2 ($s)"
sleep 30
rsync -a --inplace spark2:$CK $MD/ || fail "rsync checkpoint"
say "ckpt: $(sz "$CK") $CK"
...
rsync -a "$Q8" "$Q4" spark2:$MD/ || fail "rsync gguf to spark2"

for j in $(seq 1 60); do n=$(gpu_busy); [ "${n:-1}" -eq 0 ] && break; sleep 30; done
n=$(gpu_busy); state="clean"; [ "${n:-1}" -eq 0 ] || state="CONTENDED"
say "EAGLE3 $TAG: eval on spark2 ($state), demo llama-server co-resident"
...
for s in q8 q4s q8p5s; do
  scp -q /tmp/e3_${TAG}_$s.sh spark2:/tmp/e3_${TAG}_$s.sh && ssh spark2 "bash /tmp/e3_${TAG}_$s.sh" | grep -E '^RESULT|DONE' | tee -a "$LOG"
done
