#!/usr/bin/env bash
# EAGLE-3 scale-up run B on spark1: continue from the scale-up A EMA weights on all data
...
say "rsync half-B features from spark2"
rsync -a --inplace spark2:$V2/feats_e3/broad2_b.bin $V2/feats_e3/ || { say "rsync failed"; exit 1; }
...
sudo docker run --rm --gpus all --ipc=host --name eagle3_scaleB -v /home/usman:/home/usman bonsai/dflash-trainer:latest \
