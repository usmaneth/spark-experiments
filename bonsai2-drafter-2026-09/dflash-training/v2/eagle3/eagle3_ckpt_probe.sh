#!/usr/bin/env bash
# Quick runtime acceptance probe of an EAGLE-3 checkpoint: convert on spark1 (CPU), quantize Q4_K_M,
# push to spark2, run the three 200-token prompts at K=5 exact match. Acceptance is load-independent;
# tok/s from a contended spark2 is not.  Usage: eagle3_ckpt_probe.sh <ckpt.safetensors> <tag>
set -u
CK="${1:?ckpt}"; TAG="${2:?tag}"
ROOT=/home/usman/Bonsai-demo; V2=$ROOT/dflash-training/v2; MD=$ROOT/models/bonsai2-eagle3
...
if [ "$(nvidia-smi --query-compute-apps=pid,process_name --format=csv,noheader 2>/dev/null | grep -v llama-server | wc -l)" -eq 0 ]; then
  say "probe host: spark1 (local GPU free)"; bash /tmp/e3probe_${TAG}.sh | grep -E '^RESULT|DONE' | tee -a "$LOG"
else
  for j in $(seq 1 240); do [ "$(busy spark2)" -eq 0 ] && break; sleep 300; done
  rsync -a "$Q4" spark2:$MD/ || { say "PROBE FAILED: rsync"; exit 1; }
  say "probe host: spark2 (waited for a free GPU)"; scp -q /tmp/e3probe_${TAG}.sh spark2:/tmp/ && ssh spark2 "bash /tmp/e3probe_${TAG}.sh" | grep -E '^RESULT|DONE' | tee -a "$LOG"
fi
