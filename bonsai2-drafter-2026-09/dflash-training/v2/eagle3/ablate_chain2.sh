#!/usr/bin/env bash
# Reduced ablation chain on spark2 (runs alongside generation half b).
# Waits for the running `base` container, then: depth5, warmL0, warmL4.
# A bench window flag (/tmp/spark2_bench_window on spark1) pauses the chain between runs.
set -u
...
finish(){ # finish <name>
  ssh spark2 "sudo chown usman:p-usman $MD/bonsai2_eagle3_abl_$1*.safetensors 2>/dev/null; chmod 0644 $MD/bonsai2_eagle3_abl_$1*.safetensors 2>/dev/null"
...
run(){ # run <name> <extra flags>
  local name="$1"; shift
  while [ -f /tmp/spark2_bench_window ]; do sleep 60; done
  say "ABL $name start: $*"
...
while ssh -o ConnectTimeout=8 spark2 'sudo docker ps --format "{{.Names}}" | grep -q "^eagle3_abl_base$"'; do sleep 60; done
finish base
